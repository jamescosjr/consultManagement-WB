...
describe("GET /users/role/:role", () => {
    ...
    beforeEach(async () => {
        ...
        for (let i = 1; i <= 5; i++) {
            const password = await bcrypt.hash(`DoctorPass${i}123!`, 12);
        ...
        for (let i = 1; i <= 3; i++) {
            const password = await bcrypt.hash(`ClientPass${i}123!`, 12);
    ...
    describe("success cases", () => {
        it("should return 200 and users with specific role", async () => {
        ...
        it("should support pagination", async () => {
            const page1 = await supertest(app)
                .get('/users/role/doctor?page=1&limit=2')
                .set('Authorization', `Bearer ${rootToken}`);

            const page2 = await supertest(app)
                .get('/users/role/doctor?page=2&limit=2')
                .set('Authorization', `Bearer ${rootToken}`);
    ...
    describe("non success cases", () => {
        it("should return 401 when not authenticated", async () => {
            const response = await supertest(app).get('/users/role/doctor');
